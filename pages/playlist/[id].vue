      ...
      <div class="flex gap-4 px-4 w-full">
        ...
        <div class="flex flex-col gap-2 flex-1">
          ...
          <div class="flex gap-2 items-center">
            ...
            />
            <span class="text-sm font-bold text-[rgb(81,126,175)] cursor-pointer">{{
              playlistDetail?.creator.nickname
            }}</span>
            <span class="text-sm"
              >{{ $dayjs(playlistDetail?.createTime).format('YYYY-MM-DD') }} 创建</span
            >
          </div>
          ...
          <div class="text-sm flex gap-6">
            <p>
              <span class="text-gray-200">歌曲数:</span
              ><span class="pl-2">{{ playlistDetail?.trackCount }}</span>
            </p>
            <p>
              <span class="text-gray-200">播放数:</span
              ><span class="pl-2">{{ formatNumber(playlistDetail?.playCount) }}</span>
            </p>
          </div>
    ...
    <UTabs class="pt-4 px-8 w-full h-full" v-model="active" :panes="panes">
      <template #songs>
        <div class="grid grid-cols-[repeat(24,1fr)] h-8 gap-2 items-center text-xs font-semibold">
          ...
          <div class="col-span-7 h-full flex items-center hover:bg-neutral-700 pl-2">
            <span>音乐标题</span>
          </div>
          <div class="col-span-6 h-full flex pl-1 items-center hover:bg-neutral-700">
            <span>歌手</span>
          </div>
          <div class="col-span-5 h-full flex pl-1 items-center hover:bg-neutral-700">
            <span>专辑</span>
          </div>
          <div class="col-span-3 h-full flex pl-1 items-center hover:bg-neutral-700">
            <span>时长</span>
          </div>
        </div>
        <div
          v-for="(track, index) in playlistDetail?.tracks"
          @dblclick="playSong(track.id)"
        ...
        >
          ...
          <div class="col-span-6 h-full flex pl-1 items-center w-full">
...
function addAllSongs() {
  if (playlistDetail.value?.trackIds) addSongs([...playlistDetail.value.trackIds.map((x) => x.id)]);
}
