  ...
  <div class="px-4 pb-4">
    <UHeader>
      <div class="grid grid-cols-12 w-full h-full">
        <div class="hidden col-span-8 md:flex lg:flex items-center justify-end gap-6">
          <NuxtLink
            v-for="link in headLink"
            :to="link?.to"
            class="w-24 flex flex-col justify-center hover:cursor-pointer h-full items-center"
          >
            <div
              class="w-[80%] text-center font-bold text-base"
              :style="{ color: $route.path === link?.to ? 'red' : 'inherit' }"
            >
              {{ link.name }}
            </div>
            <div
              class="font-black text-lg relative top-4 border-b-[2px] border-b-red-600 w-[70%]"
        ...
        <div class="col-span-12 md:col-span-4 lg:col-span-4 flex justify-center items-center">
          <div class="relative flex items-center w-[70%]">
            ...
            />
            <Icon
              class="absolute flex pointer-events-none px-4 text-gray-400 dark:text-gray-500"
              size="20"
              name="ri:search-line"
            />
          </div>
    ...
    <div ref="container">
      <ClientOnly>
        ...
        >
          <Slide
            v-for="(banner, i) in banners?.filter((x) => !Boolean(x.adSource))"
            :key="banner.targetId"
            :index="i"
          >
            <NuxtImg
              class="object-fill w-full h-full"
              :src="banner.imageUrl.replace('http://', 'https://')"
              :alt="banner.typeTitle"
            />
          </Slide>
        </Carousel3d>
        <div
          class="w-full items-center md:gap-4 lg:gap-6 justify-center font-black text-3xl hidden lg:flex md:flex"
        >
          <div
            v-for="i in banners?.filter((x) => !Boolean(x.adSource)).length"
            :key="i"
            :style="{ color: index == i ? 'red' : 'inherit' }"
          >
            .
          </div>
        </div>
    ...
    <div class="flex flex-col gap-4">
          ...
          <template v-for="r in recommendResource">
            <NuxtLink :to="`/playlist/${r.id}`">
              <UCard :image="{ src: r.picUrl, alt: r.name }" :title="r.name">
                <template #hover>
                  <div
                    @click.prevent="playPlaylist(r.id)"
                    class="hidden group-hover:flex absolute bottom-4 right-4"
                  >
                    <div class="flex-1"></div>
                    <div
      ...
      <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 pt-2">
          <UCard
            :hover="false"
            v-for="r in personalizedPrivatecontent"
            :image="{ src: r.sPicUrl, alt: r.name }"
            :ui="{ image: 'aspect-video', container: 'border-none aspect-auto' }"
            :title="r.name"
          />
        </div>
      </div>
        ...
        <div class="grid grid-cols-2 grid-flow-col grid-rows-5 gap-4 pt-2">
          <template v-for="({ song }, index) in personalizedNewsong">
            <UCard
              ...
              :image="{
                src: `${song.album.picUrl.replace('http://', 'https://')}`,
                alt: song.name,
              }"
      ...
      <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 pt-2">
          <UCard
            :hover="false"
            v-for="r in personalizedMv"
            :image="{ src: r.picUrl, alt: r.name }"
            :ui="{ image: 'aspect-video', container: 'border-none aspect-auto' }"
            :title="r.name"
          />
        </div>
