...
async function fetchNeteaseClouldMusic<T = unknown>(
  ...
  const state = useState<any>(hash, () => undefined);
  if (state.value) {
    return state.value;
  }
...
export interface Playlist {
  ...
  score: any;
  algTags: any;
  distributeTags: Record<string, any>[];
  trialMode: number;
  displayTags: any;
  playlistType: string;
}

export type SiMiSongs = {
  artists: {
    name: string;
  }[];
  name: string;
...
export function personalized_privatecontent(
  params: RequestBaseConfig = {}
): Promise<{ code: number; result: Record<string, any>[]; name: string }> {
...
): Promise<{ code: number; banners: Record<string, any>[] }> {
