    ...
    >
      <div class="max-w-full m-auto w-full h-full">
        ...
        <div class="md:flex w-full md:gap-8 lg:px-24">
          <div class="w-full md:w-1/2 lg:w-1/2 flex flex-col items-center">
            ...
            >
              ...
              >
                <NuxtImg
                  ...
                  :src="`${currentSongDetail?.al.picUrl}?param=400y400`"
                  lazy="loaded"
                />
              </div>
            </div>
            <div class="hidden md:flex flex-col lg:hidden md:w-[20rem] gap-2 relative bottom-8">
              <div class="grid grid-cols-5 justify-between items-center">
                ...
                <Icon
                  class="cursor-pointer w-full text-[--text-color]"
                  :name="volumeState ? 'ri:volume-up-line' : 'ri:volume-mute-line'"
                  size="24"
                  @click="volumeToggle()"
                />
                <Icon
              ...
              <div class="grid grid-cols-5 justify-between items-center">
                ...
                <Icon
                  ...
                  @click="isCurrentlyPlayingOpen = !isCurrentlyPlayingOpen"
                  size="24"
                />
              </div>
            </div>
          ...
          <div
            class="hidden w-1/2 md:flex lg:flex flex-col flex-1 gap-4 text-[rgb(74,74,74)] dark:text-gray-200"
          >
            ...
            <div class="flex w-[70%] justify-center">
              ...
              <div class="text-sm text-[rgb(81,126,175)]">
                {{ currentSongDetail?.ar.map((x) => x.name).join('/') }}
              </div>
            </div>
            <div class="w-[70%] md:h-[32rem] lg:h-96">
              ...
              >
                <div>
                  <div
                    :class="[
                      currentActiveLyricIndex === index
                        ? 'text-base dark:text-[rgb(220,221,228)] font-bold'
                        : 'text-sm',
                      'mb-4 text-center',
                    ]"
                    ref="lyricContainer"
                    :key="index"
                    v-for="(l, index) in currentLyric"
                  >
                    {{ l.content }}
                  </div>
                </div>
          ...
          <div class="col-span-1 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              <template v-for="simiPlaylist in simiPlaylists">
                <UCard
                  ...
                  :image="{
                    src: `${simiPlaylist.coverImgUrl.replace('http://', 'https://')}`,
                    alt: simiPlaylist.description,
                  }"
                ...
                >
                  <template #title>
                    <div class="flex pl-3 flex-col justify-center gap-1 w-full">
                      <p
                        class="whitespace-nowrap text-ellipsis overflow-hidden text-sm font-medium"
                      >
                        {{ simiPlaylist.name }}
                      </p>
            ...
            <div class="flex flex-col gap-2">
              <template v-for="simiSong in simiSongs">
                <UCard
                  direction="vertical"
                  @dblclick="playSong(simiSong.id)"
                  class="group dark:hover:bg-[rgb(46,46,46)] hover:bg-[rgb(245,245,245)]"
                  :image="{
                    src: `${simiSong.album.picUrl.replace('http://', 'https://')}`,
                    alt: simiSong.name,
                  }"
                  :ui="{ image: 'w-12 h-12 max-w-none', container: 'border-none' }"
                >
                    ...
                    <div class="flex pl-3 flex-col justify-center gap-1 w-full">
                      ...
                      <div class="flex gap-1">
                        <span class="whitespace-nowrap text-ellipsis overflow-hidden">{{
                          simiSong.artists.map((x) => x.name).join('/')
                        }}</span>
                      </div>
                    </div>
                  </template>
                  <template #hover>
                    <div
                      class="hidden group-hover:flex justify-center items-center absolute w-full"
                    >
                      <div
...
const percent = computed(
  () => Math.min((currentTime.value / currentSongUrl.value?.time) * 1000, 1) || 0
);

function onPercentChange(newPercent: number) {
  audio.value!.currentTime = (currentSongUrl.value.time / 1000) * newPercent;
}
  ...
  if (newIndex !== oldIndex && newIndex !== -1 && playerModeState.value) {
    nextTick(() => {
      scrollerContainer.value
        ?.getScroller()
        .scrollToElement(lyricContainer.value[newIndex], 300, 0, true);
    });
  }
});
</script>
