          ...
          <div class="max-h-full overflow-y-auto">
            ...
            >
              <div class="col-span-6 w-full flex items-center gap-2">
                <Icon
                  v-if="song.id === currentSongId"
                  name="material-symbols:play-arrow"
                  class="text-red-600"
                />
                <p class="whitespace-nowrap text-ellipsis overflow-hidden">{{ song.name }}</p>
              </div>
              <div class="col-span-4 w-full">
                <p class="whitespace-nowrap text-ellipsis overflow-hidden">
                  {{ song.ar.map((x) => x.name).join('/') }}
                </p>
              </div>
              <div class="col-span-2 w-full">
                <span>{{ $dayjs(song.dt).format('mm:ss') }}</span>
              </div>
            </div>
          </div>
        </div>
